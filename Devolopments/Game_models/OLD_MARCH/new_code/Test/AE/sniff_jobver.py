...
class ARPSniffer:
    def __init__(self, model_path, window_size=10, alert_threshold=0.7, interface=None):
        ...
        # Буфер для хранения пакетов в текущем окне
        ...
        self.log_file = os.path.join(self.logs_dir, f"arp_alerts_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ...
    def calculate_window_features(self, window_packets):
        ...
        for packet in window_packets:
            ...
            if packet['opcode'] == 2:  # Только для ARP ответов
                ...
                if src_ip in ip_mac_mappings and ip_mac_mappings[src_ip] != src_mac:
                    changed_mappings += 1
    ...
    def rule_based_detection(self, features_dict):
        ...
        # Признаки дублирования пакетов
        if features_dict['duplicates'] > 5:
            is_attack = True
            confidence += 0.3
            
        # Множественные MAC для одного IP
        if features_dict['multiple_macs'] > 0:
            is_attack = True
            confidence += 0.5
            
        # Изменение MAC-IP связок
        if features_dict['changed_mappings'] > 0:
            is_attack = True
            confidence += 0.4
            
        # Высокая частота пакетов
        if features_dict['packet_rate'] > 3.0:
            confidence += 0.2
    ...
    def packet_handler(self, packet):
        ...
        if ARP in packet:
            ...
            window_packets = [p for p in self.packet_buffer 
                            if current_time - self.window_size <= p['timestamp'] <= current_time]
